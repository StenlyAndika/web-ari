<!-- Modal Toggle -->
<div x-data="{ edit_barang_masuk: false }">
    <!-- Open Button -->
    <button @click="edit_barang_masuk = true" class="px-4 py-2 text-white bg-primary text-sm font-bold rounded hover:bg-blue-700">
        Ubah
    </button>

    <!-- Modal Backdrop -->
    <div
        x-show="edit_barang_masuk"
        x-transition.opacity
        class="fixed inset-0 bg-black bg-opacity-50 z-40"
        @click="edit_barang_masuk = false"
    ></div>
    ...
    >
        <div
            @click.away="edit_barang_masuk = false"
            class="bg-white rounded-lg py-6 w-full max-w-md shadow-lg"
        >
            <h2 class="text-xl font-semibold mb-4 w-full border-l-4 border-emerald-500 pl-4 pr-4 py-2 bg-gray-100">Ubah Data Barang Masuk</h2>
            <form method="post" action="{{ route('admin.barang.masuk.update', $item->id) }}" autocomplete="off" class="space-y-2 px-6">
                @csrf
                @method('put')
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    ...
                    <button type="button" @click="edit_barang_masuk = false"
